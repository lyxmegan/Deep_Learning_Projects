...
class Planner(torch.nn.Module):
    ...
    def __init__(self, layers=[16, 32, 64, 128], n_input_channels=3, n_output_channels=1, kernel_size=3, use_skip=True):
        super().__init__()
        ...
        for i, l in enumerate(layers):
            self.add_module('conv%d' % i, self.Block(c, l, kernel_size=kernel_size, stride=2))
            c = l

        for i, l in list(enumerate(layers))[::-1]:
            self.add_module('upconv%d' % i, self.UpBlock(c, l, kernel_size=kernel_size, stride=2))
            c = l 
            if self.use_skip:
                c += skip_layer_size[i]
