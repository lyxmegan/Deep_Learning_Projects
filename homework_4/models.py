...
def extract_peak(heatmap, max_pool_ks=7, min_score=-5, max_det=100):
    ...
    max_cls = F.max_pool2d(heatmap[None, None], kernel_size=max_pool_ks, stride=1, padding=max_pool_ks // 2)
...
class Detector(torch.nn.Module):
    ...
    def detect(self, image):
        ...
        d1 = extract_peak(heatmap[0], max_det=30)
        ld1 = [d1[i] + (0., 0.) for i in range(len(d1))]
        d2 = extract_peak(heatmap[1], max_det=30)
        ld2 = [d2[i] + (0., 0.) for i in range(len(d2))]
        d3 = extract_peak(heatmap[2], max_det=30)
        ld3 = [d3[i] + (0., 0.) for i in range(len(d3))]
